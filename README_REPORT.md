...
## 🗺️ Product Launch Workflow

```mermaid
graph TD
    A[Raw Municipal Logs] --> B[Data & GIS Engine]
    C[OpenStreetMap Networks] --> B
    B --> D[Enforcement Bias Correction]
    D --> E[Congestion Impact Indexing]
    D --> F[Temporal LightGBM Forecasting]
    E --> G[Economic Loss Matrix]
    F --> H[Unified Command Center UI]
    G --> H
    H --> I[Proactive Patrol Dispatch]
    style A fill:#fef3c7,stroke:#d97706,stroke-width:1px
    style C fill:#fef3c7,stroke:#d97706,stroke-width:1px
    style B fill:#eff6ff,stroke:#2563eb,stroke-width:1px
    style D fill:#eff6ff,stroke:#2563eb,stroke-width:1px
    style E fill:#eff6ff,stroke:#2563eb,stroke-width:1px
    style F fill:#eff6ff,stroke:#2563eb,stroke-width:1px
    style G fill:#ecfdf5,stroke:#059669,stroke-width:1px
    style H fill:#faf5ff,stroke:#7c3aed,stroke-width:1px
    style I fill:#fff5f5,stroke:#dc2626,stroke-width:1px
```
...
* **Predictive Congestion Forecast**: Integrates a temporal autoplay slider to scrub through the 168 hours of the weekly horizon, predicting future violation spikes in specific Geohash-6 grids.
...
NammaFLOW processes raw geospatial telemetry through five pipeline stages, documented in [ml/pipeline/](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/ml/pipeline/):

### A. Timezone Normalization
All incoming logs are parsed and converted to Asia/Kolkata (IST) within [01_clean.py](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/ml/pipeline/01_clean.py). This aligns violations with the city's actual morning, midday, evening, and night commute cycles rather than raw UTC timestamps.
...
To eliminate patrol presence bias, raw violation frequencies are normalized against historical officer counts logged in that zone. The adjusted violation weight is calculated in [03_score.py](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/ml/pipeline/03_score.py) as:
...
Where:
* **$Footprint_{\text{avg}}$**: Maps the physical space occupied by the violating vehicle types (e.g. buses/trucks = 2.5m, cars = 0.8m, scooters = 0.4m), configured in [config.py](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/ml/config.py).
...
* **Algorithm**: LightGBM (Gradient Boosted Decision Trees) regressor trained in [04_forecast.py](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/ml/pipeline/04_forecast.py).
...
* **Pickled Serialization**: Compiles and serializes the model structure into a lightweight pipeline artifact (`forecast_model.pkl`) for sub-millisecond offline inferences.
...
* Performs offline network parsing using local JSON caches to avoid external API dependencies during scoring runs.
...
To justify dispatch budgets, abstract traffic delays are converted into real-world monetary damage per hour (in INR, ₹). The formulation is calculated in [03_score.py](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/ml/pipeline/03_score.py) as:
...
* **Tunable Parameters**: Allows municipal admins to edit local fuel costs and wage rates via [config.py](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/ml/config.py) to match specific cities.
...
| Layer | Component | Core Technologies | Primary Role | Key Source Files |
| :--- | :--- | :--- | :--- | :--- |
| **Data Engine & ML** | Python Pipeline | `pandas`, `NumPy`, `LightGBM`, `scikit-learn`, `Shapely`, `GeoPandas` | Data cleaning, spatial clustering, OpenStreetMap scraping, and model training. | [ml/pipeline/](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/ml/pipeline/) |
| **API Backend** | FastAPI | `FastAPI`, `Uvicorn`, In-Memory caching | Loads all static data artifacts into memory at startup to serve API requests in under 5ms. | [main.py](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/backend/main.py) |
| **Interactive UI** | React + Vite | `React`, `Tailwind CSS`, `Framer Motion`, `Lenis` | Responsive layout, sidebar menu toggles, and smooth dashboard transitions. | [App.jsx](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/frontend/src/App.jsx) |
| **GIS Mapping** | MapLibre GL | `maplibre-gl` | Client-side vector maps rendering hotspots, routing corridors, and heatmaps. | [MapView.jsx](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/frontend/src/components/MapView.jsx) |
| **3D Rendering** | Three.js WebGL | `three`, `r3f` (React Three Fiber) | WebGL rendering of the faded blueprint road structure and animated traffic flow. | [ThreeTrafficBg.jsx](file:///c:/Users/SSN/OneDrive%20-%20Shiv%20Nadar%20University%20-%20Chennai/Desktop/parksight_v3/parksight/frontend/src/components/ThreeTrafficBg.jsx) |

* **Zero-Database Overhead**: The backend utilizes dynamic caching to load pre-calculated GeoJSON states directly from Python pipelines.
...
* Spatial indexing remains extremely lightweight, scaling to millions of e-challan rows without requiring spatial joins.

To launch the system in a new city (e.g., London, New York, Mumbai), operators only need to follow three steps:
1. **Swap Input Files**: Replace the raw violation CSV in `ml/data/raw/` and extract the city's OpenStreetMap segment layout.
2. **Re-run Data Pipeline**: Run `python ml/run_all.py` to regenerate the spatial clusters, re-score the locations, and train the LightGBM forecasting model.
3. **Launch Servicers**: Spin up the FastAPI and Vite servers to serve the new city's live dispatch console.
